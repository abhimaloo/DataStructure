package com.leetcode2022.blind75.array;


/*
Suppose an array of length n sorted in ascending order is rotated between 1 and n times. For example, the array nums = [0,1,2,4,5,6,7] might become:

[4,5,6,7,0,1,2] if it was rotated 4 times.
[0,1,2,4,5,6,7] if it was rotated 7 times.
Notice that rotating an array [a[0], a[1], a[2], ..., a[n-1]] 1 time results in the array [a[n-1], a[0], a[1], a[2], ..., a[n-2]].

Given the sorted rotated array nums of unique elements, return the minimum element of this array.

You must write an algorithm that runs in O(log n) time.



Example 1:

Input: nums = [3,4,5,1,2]
Output: 1
Explanation: The original array was [1,2,3,4,5] rotated 3 times.
Example 2:

Input: nums = [4,5,6,7,0,1,2]
Output: 0
Explanation: The original array was [0,1,2,4,5,6,7] and it was rotated 4 times.
Example 3:

Input: nums = [11,13,15,17]
Output: 11
Explanation: The original array was [11,13,15,17] and it was rotated 4 times.


Constraints:

n == nums.length
1 <= n <= 5000
-5000 <= nums[i] <= 5000
All the integers of nums are unique.
nums is sorted and rotated between 1 and n times.
 */
public class FindMinimumInRotatedSortedArray {

    /*
     Idea is modified binary search
     check the pivot condition:
     nums[mid] < nums[mid-1] - return the mid
     else if (nums[mid] > nums[right]) it means the rotation is in the right subarray; hence recurse on right part
     else nums[left] > nums[mid] it means rotation is in the left subarray hence recurse on left side.
     */

    public static int findMin(int[] nums) {
        if (nums.length == 0) return -1;
        if (nums.length == 1) return nums[0];

        int pivot = binarySearch(nums, 0, nums.length - 1);
        return pivot == -1 ? nums[0] : nums[pivot];

    }

    private static int binarySearch(int[] nums, int left, int right) {
        if (left < 0 || right >= nums.length) {
            return -1;
        }

        if (left <= right) {
            int mid = (right + left) / 2;
            if (mid - 1 >= 0 && nums[mid] < nums[mid - 1]) {
                return mid;
            } else if (nums[left] <= nums[mid] && nums[mid] > nums[right]) {
                return binarySearch(nums, mid + 1, right);
            } else {
                return binarySearch(nums, left, mid - 1);
            }
        }

        return -1;
    }

    public static void main(String[] args) {
        int[] nums = new int[]{3, 4, 5, -1, 1, 2};
        System.out.println(findMin(nums));
    }

}
